...
def add_dataset():
    ...
    for one in data_list:
        ...
        if one['model'] == 'publisher':
            ...
            add_data = Stock(id=one['pk'], id_book=one['fields']['id_book'], id_shop=one['fields']['id_shop'],
                             count=one['fields']['count'])
            session.add(add_data)

        elif one['model'] == 'sale':
            add_data = Sale(id=one['pk'], price=one['fields']['price'], date_sale=one['fields']['date_sale'],
                            id_stock=one['fields']['id_stock'], count=one['fields']['count'])
            session.add(add_data)
...
def search_books():
    ...
    sq = session.query(Book.title, Shop.name, Sale.price, Sale.date_sale).join(Stock.book_s).join(Stock.shop_s).join(
        Sale).join(subq, Book.id_publisher == subq.c.id).all()
